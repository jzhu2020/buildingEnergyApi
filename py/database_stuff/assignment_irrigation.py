...
def get_last_adjustment(house_address):
    adjustments = query(conn, "Select prior_date, Water.current_date From Water \
                            Where address_street_name='{}' And address_street_number='{}' And service_id='{}' And transaction_type='Adjustment' And service_type='irrigation' \
...
def get_days(house_address):  # TODO perfect time calculations (prior and current date)
    ...
    dates = query(conn, "Select prior_date, Water.current_date From Water \
                        ...
                        And (Water.current_date>'{}' Or prior_date>'{}') And transaction_type='Charge' And service_type='irrigation' \
...
households = query(conn, "Select Distinct address_street_name, address_street_number, service_id From Water Where service_type='irrigation' ")
streets = query(conn, "Select Distinct address_street_name From Water Where service_type='irrigation' ")
...
for house in households:
    ...
    result = query(conn, "Select current_reading, prior_reading From Water Where address_street_name='{}' \
                        ...
                        And service_type='irrigation' And prior_date<Water.current_date Order By prior_date, Water.current_date Asc;".format(house[0], house[1], house[2], last_adjustment, last_adjustment))
...
for street in streets:
    ...
    houses = query(conn, "Select Distinct address_street_name, address_street_number, service_id From Water \
                         Where address_street_name='{}' And service_type='irrigation';".format(street[0]))
    for house in houses:
...
print_to_csv("IrrigationAverageGPD.csv", household_data)
print_to_csv("IrrigationStreetAverageGPD.csv", street_data)

...
df = df_census.drop( columns='id' )
# df.to_excel( 'census.xlsx', index=False )
df.to_csv( 'census.csv', index=False, header=True )

# Write Solar dataframe to Excel and CSV files
df = df_solar.drop( columns='id' )
# df.to_excel( 'solar.xlsx', index=False )
df.to_csv( 'solar.csv', index=False, header=True )
